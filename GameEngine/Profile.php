...
class Profile 
{
    ...
    private function updateProfile($post)
    {
        ...
        $this->database->submitProfile(
            $this->database->RemoveXSS($post['uid']),
            $this->database->RemoveXSS($post['mw']),
            $this->database->RemoveXSS($post['ort']),
            $this->database->RemoveXSS($birthday),
            $this->database->RemoveXSS($post['be2']),
            $this->database->RemoveXSS($post['be1'])
        );

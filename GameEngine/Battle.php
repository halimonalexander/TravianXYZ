...
class Battle
{
    ...
    private function getBattleHero($uid)
    {
        ...
        $herodata = GlobalVariablesHelper::getHeroStats($heroarray[0]['unit']);
        
        if (!isset($heroarray['health']))
            $heroarray['health'] = 0;
    ...
    private function simulate($post)
    {
        ...
        if(!$scout)
            return $this->calculateBattle(
                $attacker,$defender,
                $wall,$post['a1_v'],$deftribe,$palast,
                $post['ew1'],$post['ew2'],$post['ktyp']+3,$def_ab,
                $att_ab1,$att_ab2,$att_ab3,$att_ab4,$att_ab5,$att_ab6,$att_ab7,$att_ab8,
                $post['kata'],$post['stonemason'],
                $walllevel,$offhero,$post['h_off'],$deffhero,
                0,0,0,0,0
            );
        else
            return $this->calculateBattle(
    ...
    {
        $bid34 = GlobalVariablesHelper::getBuilding(Buildings::STONEMASONS_LODGE);
        $bid35 = GlobalVariablesHelper::getBuilding(Buildings::BREWERY);
        ...
        $att_artefact = count($this->database->getOwnUniqueArtefactInfo2($AttackerID,3,3,0));
        $att_artefact1 = count($this->database->getOwnUniqueArtefactInfo2($AttackerWref,3,1,1));
        $att_artefact2 = count($this->database->getOwnUniqueArtefactInfo2($AttackerID,3,2,0));
        
        if ($att_artefact > 0) {
            $attacker_artefact = 10;
        } elseif($att_artefact1 > 0){
            $attacker_artefact = 5;
        ...
        $def_artefact = count($this->database->getOwnUniqueArtefactInfo2($DefenderID,3,3,0));
        $def_artefact1 = count($this->database->getOwnUniqueArtefactInfo2($DefenderWref,3,1,1));
        $def_artefact2 = count($this->database->getOwnUniqueArtefactInfo2($DefenderID,3,2,0));
        ...
        $DefendersAll = $this->database->getEnforceVillage($DefenderWref,0);
        if (!empty($DefendersAll) && $DefenderWref>0) {
            foreach($DefendersAll as $defenders) {
                for ($i=1; $i<=50;$i++) {$def_ab[$i]=0;}
                $fromvillage = $defenders['from'];
                $enforcetribe = $this->database->getUserField($this->database->getVillageField($fromvillage,"owner"),"tribe",0);
        ...
        if($type == 1) {//scout
            for($i=$start;$i<=$end;$i++) {
                $unit = GlobalVariablesHelper::getUnit($i);
                $index = 'u' . $i;
                $j = $i - $start + 1;
                if($Attacker[$index]>0 && ($i == 4 || $i == 14 || $i == 23 || $i == 44)){
                    if(${'att_ab'.$abcount} > 0) {
                        $ap += (35 + (35 + 300 * $unit['pop'] / 7) * (pow(1.00697, ${'att_ab'.$abcount}) - 1)) * $Attacker[$index];// ^ ($Attacker['u'.$i]/100);
                    }else{
                        $ap += $Attacker[$index]*35;
                    }
                }
                $involve += $Attacker[$index];
                $units['Att_unit'][$i] = $Attacker[$index];
            }

            $att_foolartefact = $this->database->getFoolArtefactInfo(3,$AttackerWref,$AttackerID);
            ...
            for($i=$start;$i<=$end;$i++) {
                ...
                if ($abcount <= 8 && ${'att_ab'.$abcount} > 0) {
                    if(in_array($i,$calvary)) {
                        $cap += ($unit['atk'] + ($unit['atk'] + 300 * $unit['pop'] / 7) * (pow(1.007, ${'att_ab'.$abcount}) - 1)) * $Attacker[$index];
                    }else{
                        $ap += ($unit['atk'] + ($unit['atk'] + 300 * $unit['pop'] / 7) * (pow(1.007, ${'att_ab'.$abcount}) - 1)) * $Attacker[$index];
                    }
                }else{
                    if(in_array($i,$calvary)) {
                        $cap += $Attacker[$index] * $unit['atk'];
                    }else{
                        $ap += $Attacker[$index] * $unit['atk'];
                    }
                }
                $abcount +=1;
        ...
        if($catp > 0 && $tblevel != 0) {
            ...
            $artowner = $this->database->getVillageField($DefenderWref,"owner");
            $bartefact = count($this->database->getOwnUniqueArtefactInfo2($artowner,1,3,0));
            $bartefact1 = count($this->database->getOwnUniqueArtefactInfo2($DefenderWref,1,1,1));
            $bartefact2 = count($this->database->getOwnUniqueArtefactInfo2($artowner,1,2,0));
            if($bartefact > 0){
                $strongerbuildings = 5;
            }elseif($bartefact1 > 0){
                $strongerbuildings = 4;
            ...
            $foolartefact = $this->database->getFoolArtefactInfo(3,$DefenderWref,$artowner);
        ...
        if (isset($units['Att_unit']['hero']) && $units['Att_unit']['hero'] >0){
            $_result = $this->database->query("select * from " . TB_PREFIX . "hero where `dead`='0' and `heroid`='".$atkhero['heroid']."'");
            $fdb = $this->database->fetchArray($_result);
            $hero_id=$fdb['heroid'];
            ...
            if ($hero_health<=$damage_health or $damage_health>90){
                ...
                $this->database->query("update " . TB_PREFIX . "hero set `dead`='1' where `heroid`='".$hero_id."'");
                $this->database->query("update " . TB_PREFIX . "hero set `health`='0' where `heroid`='".$hero_id."'");
            }else{
                $this->database->query("update " . TB_PREFIX . "hero set `health`=`health`-".$damage_health." where `heroid`='".$hero_id."'");
            }
        }
        ...
        if (isset($units['Def_unit']['hero']) && $units['Def_unit']['hero'] >0){
            $_result=$this->database->query("select * from " . TB_PREFIX . "hero where `dead`='0' and `heroid`='".$defenderhero['heroid']."'");
            $fdb = $this->database->fetchArray($_result);
            $hero_id=$fdb['heroid'];
            $hero_health=$fdb['health'];
        ...
        if(!empty($DefendersAll)){
            foreach($DefendersAll as $defenders) {
                if($defenders['hero']>0) {
                    ...
                    $Reinforcer = $this->database->getVillageField($defenders['from'],"owner");
                    $heroarraydefender = $this->getBattleHero($Reinforcer);
                    $_result=$this->database->query("select * from " . TB_PREFIX . "hero where `dead`='0' and `heroid`='".$heroarraydefender['heroid']."'");
                    $fdb = $this->database->fetchArray($_result);
                    $hero_id=$fdb['heroid'];
                    $hero_health=$fdb['health'];
                    $damage_health=round(100*$result[2]);
        ...
        for ($i=$start;$i<=$end;$i++) {
            ...
            $max_bounty += ($Attacker[$index]-$result['casualties_attacker'][$y])*$unit['cap'];
        }
    ...
    public function getDataDefScout($defenders,$def_ab,$defender_artefact,$AttackerWref,$AttackerID)
    {
        ...
        for($y=4;$y<=54;$y++) {
            if($y == 4 || $y == 14 || $y == 23 || $y == 44){
                $unit = GlobalVariablesHelper::getUnit($y);
                $index = 'u'.$y;
                
                if($defenders[$index]>0 && $def_ab[$y] > 0) {
                    $dp +=  (20 + (20 + 300 * $unit['pop'] / 7) * (pow(1.00696, $def_ab[$y]) - 1)) * $defenders[$index] * $defender_artefact;

                    $def_foolartefact = $this->database->getFoolArtefactInfo(3,$AttackerWref,$AttackerID);
                    ...
                    if($defenders[$index]>0) {
                        $dp +=  $defenders[$index]* 20 * $defender_artefact;
                    }
                    
                    $units['Def_unit'][$y] = $defenders[$index];
                    if($units['Def_unit'][$y] > 0){
                        $detected = 1;
                    }
                }
                $invol += $defenders[$index]; //total troops
                $units['Def_unit'][$y] = $defenders[$index];
            }
        }
    ...
    public function getDataDef($defenders,$def_ab)
    {
        ...
        for ($y = 1; $y <= 50; $y++) {
            ...
            if ($defenders[$index] > 0) {
                if ($def_ab[ $y ] > 0) {
                    $dp += ($unit['di'] + ($unit['di'] + 300 * $unit['pop'] / 7) * (pow(1.007, $def_ab[ $y ]) - 1)) * $defenders[$index];
                    $cdp += ($unit['dc'] + ($unit['dc'] + 300 * $unit['pop'] / 7) * (pow(1.007, $def_ab[ $y ]) - 1)) * $defenders[$index];
                } else {
                    $dp += $defenders[$index] * $unit['di'];
                    $cdp += $defenders[$index] * $unit['dc'];
                }
            }

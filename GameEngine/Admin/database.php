...
class adm_DB
{
    ...
    public function Login($username,$password)
    {
        ...
        if ($dbarray['password'] == md5($password)) {
            $this->connection->query("Insert into ".TB_PREFIX."admin_log values (0,'X','$username logged in (IP: <b>".$_SERVER['REMOTE_ADDR']."</b>)',".time().")");
            return true;
        }
        else {
            $this->connection->query("Insert into ".TB_PREFIX."admin_log values (0,'X','<font color=\'red\'><b>IP: ".$_SERVER['REMOTE_ADDR']." tried to log in with username <u> $username</u> but access was denied!</font></b>',".time().")");
            return false;
        }
    }
    ...
    public function buildingCP($f,$lvl)
    {
        $name = "bid" . $f;
        global $$name;
        $popT = 0;
    ...
    public function AddVillage($post)
    {
        ...
        $status = 0;
        if ($status == 0) {
            $this->connection->query("Insert into " . TB_PREFIX . "admin_log values (0," . $_SESSION['id'] . ",'Added new village <b><a href=\'admin.php?p=village&did=$wid\'>$wid</a></b> to user <b><a href=\'admin.php?p=player&uid=$uid\'>$uid</a></b>'," . time() . ")");
    ...
    public function Punish($post)
    {
        ...
        $this->connection->query("Insert into " . TB_PREFIX . "admin_log values (0," . $_SESSION['id'] . ",'Punished user: <a href=\'admin.php?p=player&uid=" . $post['uid'] . "\'>" . $post['uid'] . "</a> with <b>-" . $post['punish'] . "%</b> population'," . time() . ")");
    }
    ...
    public function DelPlayer($uid,$pass)
    {
        ...
        if ($this->CheckPass($pass, $ID)) {
            ...
            $this->connection->query("Insert into " . TB_PREFIX . "admin_log values (0,$ID,'Deleted user <a>$name</a>'," . time() . ")");
    ...
    public function DelVillage($wref, $mode=0)
    {
        ...
        if (mysqli_num_rows($result) > 0) {
            $this->connection->query("Insert into " . TB_PREFIX . "admin_log values (0," . $_SESSION['id'] . ",'Deleted village <b>$wref</b>'," . time() . ")");
    ...
    public function DelBan($uid, $id)
    {
        ...
        $this->connection->query("Insert into " . TB_PREFIX . "admin_log values (0," . $_SESSION['id'] . ",'Unbanned user <a href=\'admin.php?p=player&uid=$uid\'>$name</a>'," . time() . ")");
    ...
    public function AddBan($uid,$end,$reason)
    {
        ...
        $name = addslashes($database->getUserField($uid, "username", 0));
        $this->connection->query("Insert into " . TB_PREFIX . "admin_log values (0," . $_SESSION['id'] . ",'Banned user <a href=\'admin.php?p=player&uid=$uid\'>$name</a>'," . time() . ")");
    ...
    public function returnTroops($wref)
    {
        ...
        foreach ($getenforce as $enforce) {
            ...
            elseif ($database->getUserField($to['owner'], 'tribe', 0) == '3') {
                $Gtribe = "2";
            }
            elseif ($database->getUserField($to['owner'], 'tribe', 0) == '4') {
                $Gtribe = "3";
            }
            elseif ($database->getUserField($to['owner'], 'tribe', 0) == '5') {
                $Gtribe = "4";
            }
    ...
    public function calculateProduction($wid,$uid,$b1,$b2,$b3,$b4,$fdata,$ocounter,$pop)
    {
        ...
        elseif ($largeA['size'] == 2 && $largeA['owner'] == $uid) {
            $production['crop'] = $this->getCropProd($fdata, $ocounter, $b4) - $pop - (($upkeep) - round($upkeep * 0.25));
        }
        else {
    ...
    private function getAllUnits($base,$InVillageOnly=False,$mode=0)
    {
        global $database;
...
$admin = new adm_DB($database->connection);

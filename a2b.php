...
<?php
		...
		if(!empty($id)) {
			...
			$database->addMovement(MovementTypeSid::RETURNING,$prisoner['wref'],$prisoner['from'],$p_reference,time(),($p_time+time()));
			...
			if($prisoner['t11'] > 0){
			$p_owner = $database->getVillageField($prisoner['from'],"owner");
			$database->query("UPDATE ".TB_PREFIX."hero SET `dead` = '1', `health` = '0' WHERE `uid` = '".$p_owner."'");
			}
			$database->modifyUnit($prisoner['wref'],array("99o"),array($troops),array(0));
			$database->deletePrisoners($prisoner['id']);
				}

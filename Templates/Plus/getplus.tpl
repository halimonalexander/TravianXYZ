...
EOT;
    ...
    $account = $database->realEscapeString($_POST['username']);
    $reward = $database->realEscapeString($_POST['reward']);
    $valid=TRUE;
...
/////////////////////////////////////////////////////////
    ...
    $accountCheck = $database->query("SELECT * FROM ".TB_PREFIX."users WHERE `id`='".$session->uid."'");
    if($database->numRows($accountCheck) <= 0){
        echo "<b>ERROR:</b><br />";
    ...
    if(!$valid) break;
    $valid=TRUE;
    $acc = $database->fetchArray($accountCheck);

    $plusCheck = $database->query("SELECT * FROM ".TB_PREFIX."users WHERE `id`='".$session->uid."'");
    $pluss = $database->fetchArray($plusCheck);
...
echo' Please select the option you wish to activate or extend.<br>';
    ...
    if($database->numRows($plusCheck) > 0){ 
        if($time > $pluss[$key] ){
            $editplus = $database->query("UPDATE ".TB_PREFIX."users SET `{$key}`= `{$key}` + ('".$time."'+'".$plusTime."'),  `gold` =  `gold` - {$gldz}   WHERE `id`='".$session->uid."'");
        ...
        if($time < $pluss[$key]){
            $editplus = $database->query("UPDATE ".TB_PREFIX."users SET `{$key}`= `{$key}` +'".$plusTime."',  `gold` =  `gold` - {$gldz}  WHERE `id`='".$session->uid."'");
            echo "<META HTTP-EQUIV=Refresh CONTENT=\"2; {$url}\" ><br /><br /><div align=center><font color=green size=4><b> Your Status has been updated!</b></font></div>";
       
       }
    }
else{
        $insertplus = $database->query("INSERT INTO ".TB_PREFIX."users (`username`,`{$key}`, `gold`) VALUES ('".$session->username."', ('".$time."'+'".$plusTime."'),`gold` - {$gldz})");

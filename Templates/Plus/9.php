    ...
    if ($session->sit == 0) {
        if ($database->numRows($MyGold)) {
            if ($golds['6'] > 2) {
                if ($database->numRows($MyGold)) {
                    if ($golds['b1'] < time()) {
                        $database->query("UPDATE " . TB_PREFIX . "users set b1 = '" . (time() + PLUS_PRODUCTION) . "' where `id`='" . $session->uid . "'");
                    }
                    else {
                        $database->query("UPDATE " . TB_PREFIX . "users set b1 = '" . ($golds['b1'] + PLUS_PRODUCTION) . "' where `id`='" . $session->uid . "'");
                    }
                    
                    $done1 = "+25% Production: Lumber";
                    $database->query("UPDATE " . TB_PREFIX . "users set gold = " . ($session->gold - 5) . " where `id`='" . $session->uid . "'");
                    $database->query("INSERT INTO " . TB_PREFIX . "gold_fin_log VALUES ('" . ($database->numRows($goldlog) + 1) . "', '" . $village->wid . "', '+25%  Production: Lumber')");
                }
                else {
                    $done1 = "nothing has been done";
                    $database->query("INSERT INTO " . TB_PREFIX . "gold_fin_log VALUES ('" . ($database->numRows($goldlog) + 1) . "', '" . $village->wid . "', 'Failed +25%  Production: Lumber')");
                }
            }
            else {
                $done1 = "You need more gold";

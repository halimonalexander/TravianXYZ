...
if (WW == True) 
{ 
    $result = $database->query("select " . TB_PREFIX . "users.id, " . TB_PREFIX . "users.username," . TB_PREFIX . "users.alliance, " . TB_PREFIX . "fdata.wwname, " . TB_PREFIX . "fdata.f99, " . TB_PREFIX . "vdata.name, " . TB_PREFIX . "fdata.vref  
...
<table cellpadding="1" cellspacing="1" id="villages" class="row_table_data"> 
    ...
    <tbody>  
        ...
        { 
            ...
            $query = @$database->query('SELECT * FROM `' . TB_PREFIX . 'ww_attacks` WHERE `vid` = ' . $row['vref'] . ' ORDER BY `attack_time` ASC LIMIT 1'); 
            $row2 = $database->fetchAssoc($query);
        ?> 
        <tr> 
              <td><?php echo $cont; $cont++; ?>.</td> 
            <td><?php echo "<a href=\"" . \App\Routes::MAP . "?d=" . $row['vref'] . "&amp;c=" . $generator->getMapCheck($row['vref']) . "\">"; ?><?php echo $row['username']; ?></a></td>

...
foreach($varray as $vil){
	...
	echo '<td class="cel">'.($lvlTH>0?'<a href="'.\App\Routes::BUILD.'?newdid='.$vid.'&amp;gid=24">'.($hasCel<>0?'<span id="timer'.$timer.'">'.\App\Helpers\DatetimeHelper::secondsToTime($hasCel-time()).'</span>':'●').'</a>':'&nbsp;').'</td>';

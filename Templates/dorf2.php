...
<div id="village_map" class="<?php echo $vmapc; ?>">
    ...
    <div id="levels" <?php if(isset($_COOKIE['t3l'])) { echo "class=\"on\""; } ?> >
        ...
        for($i=1; $i<=20; $i++) {
            $rand = rand(0,3);
            $ttt = ['', 'lvl-tooltip-available', 'lvl-tooltip-unavailable', 'lvl-tooltip-prohibited'];
            $rand2 = rand(0,1);
            $ttb = ['', 'lvl-tooltip-underConstruction'];
          
            if ($village->resarray['f' . ($i+18) . 't'] != 0) {
                echo "<div class=\"lvl-tooltip {$ttt[$rand]} {$ttb[$rand2]} d$i\">".$village->resarray['f'.($i+18)]."</div>";
            }
        }
        
        if($village->resarray['f39t'] != 0) {
            echo "<div class=\"lvl-tooltip lvl-tooltip-available l39\">".$village->resarray['f39']."</div>";
        }
        
        if($village->resarray['f40t'] != 0) {
            echo "<div class=\"lvl-tooltip lvl-tooltip-unavailable l40\">".$village->resarray['f40']."</div>";
        }
        
        if($village->resarray['f99t'] != 0) {
            echo "<div class=\"lvl-tooltip lvl-tooltip-unavailable d40\">".$village->resarray['f99']."</div>";
        }
        ?>
    </div>

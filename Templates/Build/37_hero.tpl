...
if (isset($_POST['name'])) { 
	$_POST['name'] = stripslashes($_POST['name']);
     $database->query("UPDATE ".TB_PREFIX."hero SET `name`='".($_POST['name'])."' where `uid`='".$session->uid."'"); 
        $hero = $database->query("SELECT * FROM " . TB_PREFIX . "hero WHERE `uid` = " . $session->uid . ""); 
        $hero_info = $database->fetchArray($hero); 
        echo "".NAME_CHANGED.""; 
    } 
    ...
    if(isset($_GET['add'])) { 
            if($_GET['add'] == "reset") { 
                if($hero_info['level'] <= 3){ 
                    ...
                    $database->query("UPDATE " . TB_PREFIX . "hero SET `points` = '".(($hero_info['level']*5)+5)."' WHERE `uid` = '" . $session->uid . "'"); 
                    $database->query("UPDATE " . TB_PREFIX . "hero SET `attack` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $database->query("UPDATE " . TB_PREFIX . "hero SET `defence` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $database->query("UPDATE " . TB_PREFIX . "hero SET `attackbonus` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $database->query("UPDATE " . TB_PREFIX . "hero SET `defencebonus` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $database->query("UPDATE " . TB_PREFIX . "hero SET `regeneration` = '0' WHERE `uid` = '" . $session->uid . "'"); 
            ...
            if($_GET['add'] == "off" && $hero_info['attack'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `attack` = `attack` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: build.php?id=".$id.""); 
                    } 
                } 
            if($_GET['add'] == "deff" && $hero_info['defence'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `defence` = `defence` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: build.php?id=".$id.""); 
                    } 
                } 
          if($_GET['add'] == "obonus" && $hero_info['attackbonus'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `attackbonus` = `attackbonus` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: build.php?id=".$id.""); 
                    } 
                } 
          if($_GET['add'] == "dbonus" && $hero_info['defencebonus'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `defencebonus` = `defencebonus` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: build.php?id=".$id.""); 
                    } 
                } 
          if($_GET['add'] == "reg" && $hero_info['regeneration'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `regeneration` = `regeneration` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: build.php?id=".$id.""); 
                    } 
                } 

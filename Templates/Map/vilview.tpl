...
$result = $database->query("SELECT * FROM ".TB_PREFIX."ndata WHERE $limit AND ally = ".$session->alliance." AND toWref = ".$toWref." ORDER BY time DESC Limit 5");
...
$result = $database->query("SELECT * FROM ".TB_PREFIX."ndata WHERE uid = ".$session->uid." AND toWref = ".$toWref." ORDER BY time DESC Limit 5");
$query = $database->numRows($result);
if($query != 0){
while($row = $database->fetchArray($result)){
	$dataarray = explode(",",$row['data']);
...
<table cellpadding="1" cellspacing="1" id="options" class="tableNone">
	...
	?>
        ...
        <tr>
          ...
          $query1 = $database->query('SELECT * FROM `' . TB_PREFIX . 'odata` WHERE `wref` = ' . $database->realEscapeString($_GET['d']));
		  }else{
          $query1 = $database->query('SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `wref` = ' . $database->realEscapeString($_GET['d']));
		  }

...
if ($database->numRows($database->query("SELECT id FROM ".TB_PREFIX."users WHERE access = 9 AND id = ".$session->uid)) != '1') die("Hacking attemp!");
...
if (isset($_GET['send']) && isset($_GET['from']))
{
	...
	$users_count = $database->fetchAssoc($database->query("SELECT count(*) as count FROM ".TB_PREFIX."users WHERE id != 0"));

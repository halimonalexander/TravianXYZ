...
class BuildingController extends AbstractVillageController
{
    public function displayAction()
    {
        ...
        $this->old(
            ...
            (Registry::getInstance())->get('units')
        );
        ...
        $this->loadTemplate('building', [
            ...
            'market'     => (Registry::getInstance())->get('market'),
    ...
    private function old(
        ...
        if ($this->session->goldclub == 1 && count($this->session->villages) > 1){
            ...
            if (isset($_POST['action']) && $_POST['action'] == 'addRoute') {
                if($this->session->access != BANNED){
                    if($this->session->gold >= 2) {
                        ...
                        if($totalres > 0){
                            $this->database->createTradeRoute(
                                $this->session->uid,
                                $_POST['tvillage'],
                                $village->wid,
            ...
            if(isset($_GET['action']) && $_GET['action'] == 'extendRoute') {
                if($this->session->access != BANNED){
                    if($this->session->gold >= 2) {
                        $traderoute = $this->database->getTradeRouteUid($_GET['routeid']);
                        if($traderoute == $this->session->uid){
                            $this->database->editTradeRoute($_GET['routeid'],"timeleft",604800,1);
                            $newgold = $this->session->gold-2;
            ...
            if(isset($_POST['action']) && $_POST['action'] == 'editRoute') {
                if($this->session->access != BANNED){
                    ...
                    $reqMerc = ceil(($totalres-0.1)/$market->maxcarry);
                    if($totalres > 0){
                        $this->database->editTradeRoute($_POST['routeid'],"wood",$_POST['r1'],0);
                        $this->database->editTradeRoute($_POST['routeid'],"clay",$_POST['r2'],0);
                        $this->database->editTradeRoute($_POST['routeid'],"iron",$_POST['r3'],0);
                        $this->database->editTradeRoute($_POST['routeid'],"crop",$_POST['r4'],0);
                        $this->database->editTradeRoute($_POST['routeid'],"start",$_POST['start'],0);
                        $this->database->editTradeRoute($_POST['routeid'],"deliveries",$_POST['deliveries'],0);
                        $this->database->editTradeRoute($_POST['routeid'],"merchant",$reqMerc,0);
                        $second = date("s");
                        $minute = date("i");
                        $hour = date("G")-$_POST['start'];
                        if(date("G") > $_POST['start']){
                        ...
                        $this->database->editTradeRoute($_POST['routeid'],"timestamp",$timestamp,0);
                    }
                    header("Location: " . \App\Routes::BUILD . "?gid=17&t=4");
                    $route = 1;
                    unset($routeid);
            ...
            if(isset($_GET['action']) && $_GET['action'] == 'delRoute') {
                if($this->session->access != BANNED){
                    $traderoute = $this->database->getTradeRouteUid($_GET['routeid']);
                    if($traderoute == $this->session->uid){
                        $this->database->deleteTradeRoute($_GET['routeid']);
                        header("Location: " . \App\Routes::BUILD . "?gid=17&t=4");
                        $route = 1;
                        unset($routeid);
                    }else{
        ...
        if ($this->session->goldclub == 1){
            if (isset($_GET['t'])==99) {
                ...
                if($_GET['slid']) {
                    $FLData = $this->database->getFLData($_GET['slid']);
                    if($FLData['owner'] == $this->session->uid){
                        $checked[$_GET['slid']] = 1;
                    }
                }
            
                if($_GET['action'] == 'deleteList') {
                    $this->database->delFarmList($_GET['lid'], $this->session->uid);
                    header("Location: " . \App\Routes::BUILD . "?id=39&t=99");
                }elseif($_GET['action'] == 'deleteSlot') {
                    $this->database->delSlotFarm($_GET['eid']);
                    header("Location: " . \App\Routes::BUILD . "?id=39&t=99");
                }
                ...
                if(isset($_GET['evasion']) && is_numeric($_GET['evasion'])) {
                    $evasionvillage = $this->database->getVillage($_GET['evasion']);
                
                    if($evasionvillage['owner'] == $this->session->uid){
                        $this->database->setVillageEvasion($_GET['evasion']);
                    }
                    header("Location: " . \App\Routes::BUILD . "?id=39&t=99");
                }
            
                if(isset($_POST['maxevasion']) && is_numeric($_POST['maxevasion'])) {
                    $this->database->updateUserField($this->session->uid, "maxevasion", $_POST['maxevasion'], 1);
                    header("Location: " . \App\Routes::BUILD . "?id=39&t=99");
                }
            }
        }else{
        ...
        if($_GET['mode']=='troops' && $_GET['cancel']==1){
            if($this->session->access != BANNED){
                $oldmovement=$this->database->getMovementById($_GET['moveid']);
                $now=time();
                if(($now-$oldmovement[0]['starttime'])<90 && $oldmovement[0]['from'] == $village->wid){
                    ...
                    $resultc=$this->database->query($qc);
                
                    if ($this->database->numRows($resultc)==1){
                        ...
                        $lastid = $this->database->fetchArray($this->database->query($q2));
                        $newid = $lastid['id']+1;
                    
                        $q2 = "INSERT INTO " . TB_PREFIX . "send values ($newid,0,0,0,0,0)";
                        $this->database->query($q2);
                    
                        $this->database->addMovement(MovementTypeSid::RETURNING,$oldmovement[0]['to'],$oldmovement[0]['from'],$oldmovement[0]['ref'],$now,$end);
                        $this->database->addMovement(MovementTypeSid::CHEF_TAKEN,$oldmovement[0]['to'],$oldmovement[0]['from'],$newid,$now,$end);
                    }
                }

<div id="side_navi">
    <a id="logo" href="<?=HOMEPAGE?>" title="logo">
        <img src="/img/x.gif" <?=($this->session->plus ? 'class="logo_plus"' :'')?> alt="Travian">
    </a>
    
    <p>
        <a href="<?=HOMEPAGE?>"><?=HOME?></a>
        <a href="/spieler.php?uid=<?php echo $this->session->uid; ?>"><?=PROFILE?></a>
        <a href="#" onclick="return Popup(0,0,1);"><?=INSTRUCT?></a>
        
        <?php if ($this->session->access == MULTIHUNTER) { ?>
    ...
    <?php
    $isDeletingAccountTimestamp = $this->database->isDeleting($this->session->uid);
    if ($isDeletingAccountTimestamp) {
        ...
        <p>
            <?php if ($canCancelDeletion) { ?>
                <a href="/spieler.php?s=3&id=<?=$this->session->uid?>&a=1&e=4">

<?php
include("next.php");
      		$artefact_2 = count($this->database->getOwnUniqueArtefactInfo2($session->uid,7,3,0));
			$artefact1_2 = count($this->database->getOwnUniqueArtefactInfo2($village->wid,7,1,1));
			$artefact2_2 = count($this->database->getOwnUniqueArtefactInfo2($session->uid,7,2,0));
			if($artefact_2 > 0){
			$artefact_bouns = 6;
			...
			$foolartefact = $this->database->getFoolArtefactInfo(7,$village->wid,$seesion->uid);

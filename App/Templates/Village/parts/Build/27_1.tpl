<?php
$artefact1 = $this->database->getOwnArtefactInfo3($session->uid);
$wref = $village->wid;
$coor = $this->database->getCoor($wref);
...
if (count($artefact1)==0){
                ...
                $coor2 = $this->database->getCoor($artefact['vref']);
...
$effect = "account";
        ...
        echo '<td class="nam">
<a href="'.\App\Routes::BUILD.'?id=' . $id . '&show='.$artefact['id'].'">' . $artefact['name'] . '</a> <span class="bon">' . $artefact['effect'] . '</span>
<div class="info">
...
if($this->database->numRows($this->database->query("SELECT * FROM " . TB_PREFIX . "artefacts")) == 0) {
    echo '<td colspan="4" class="none">'.NO_ARTIFACTS_AREA.'</td>';
        ...
        unset($reqlvl);
        unset($effect);
        $arts = $this->database->query("SELECT * FROM " . TB_PREFIX . "artefacts");
        $rows = array();
        while($row = $this->database->fetchArray($arts)) {
            $query = $this->database->query('SELECT * FROM `' . TB_PREFIX . 'wdata` WHERE `id` = ' . $row['vref']);
            $coor2 = $this->database->fetchAssoc($query);
        ...
        foreach($rows as $row) {
                ...
                $coor = $this->database->getCoor($wref);
                $wref2 = $row['vref'];
                $coor2 = $this->database->getCoor($wref2);
                        echo '<tr>';
                        echo '<td class="icon"><img class="artefact_icon_' . $row['type'] . '" src="img/x.gif" alt="" title=""></td>';
                        echo '<td class="nam">';
                        echo '<a href="'.\App\Routes::BUILD.'?id=' . $id . '&show='.$row['id'].'">' . $row['name'] . '</a> <span class="bon">' . $row['effect'] . '</span>';
                        echo '<div class="info">';

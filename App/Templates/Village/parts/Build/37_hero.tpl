...
if (isset($_POST['name'])) { 
	$_POST['name'] = stripslashes($_POST['name']);
     $this->database->query("UPDATE ".TB_PREFIX."hero SET `name`='".($_POST['name'])."' where `uid`='".$session->uid."'"); 
        $hero = $this->database->query("SELECT * FROM " . TB_PREFIX . "hero WHERE `uid` = " . $session->uid . ""); 
        $hero_info = $this->database->fetchArray($hero); 
        echo "".NAME_CHANGED.""; 
    } 
...
<table id="distribution" cellpadding="1" cellspacing="1"> 
		...
		<th colspan="5">
			<?php  
				if(isset($_GET['rename'])){ 
					...
					echo "<a href=\"".\App\Routes::BUILD."?id=".$id."&rename\">".$hero_info['name']."</a></form>";
				} 
			?>
    ...
    <tbody><tr> 
		...
		if($session->access != BANNED){
            echo "<a href=\"".\App\Routes::BUILD."?id=".$id."&add=off\">(<b>+</b>)</a>";
		}else{
    ...
    <tr> 
		...
		if($session->access != BANNED){
            echo "<a href=\"".\App\Routes::BUILD."?id=".$id."&add=deff\">(<b>+</b>)</a>";
		}else{
		header("Location: banned.php"); 
		}
        }else { 
		...
		if($session->access != BANNED){
            echo "<a href=\"".\App\Routes::BUILD."?id=".$id."&add=obonus\">(<b>+</b>)</a>";
		}else{
		header("Location: banned.php"); 
		}			
    ...
    <tr> 
		...
		if($session->access != BANNED){
            echo "<a href=\"".\App\Routes::BUILD."?id=".$id."&add=dbonus\">(<b>+</b>)</a>";
		}else{
		header("Location: banned.php"); 
		}
    ...
    <tr> 
		...
		if($session->access != BANNED){
            echo "<a href=\"".\App\Routes::BUILD."?id=".$id."&add=reg\">(<b>+</b>)</a>";
		}else{
		header("Location: banned.php"); 
		}
    ...
    <?php if($hero_info['level'] <= 3){ ?> 
        <p><?php echo YOU_CAN; ?> <a href="<?=\App\Routes::BUILD?>?id=<?php echo $id; ?>&add=reset"><?php echo RESET; ?></a><?php echo YOUR_POINT_UNTIL; ?> <b>3</b><?php echo OR_LOWER; ?> </p>
    <?php } ?> 
     
<p><?php echo YOUR_HERO_HAS; ?> <b><?php echo floor($hero_info['health']); ?></b>% <?php echo OF_HIT_POINTS; ?>.<br/>  
    <?php echo YOUR_HERO_HAS; ?> <?php echo CONQUERED; ?> <b><?php echo $this->database->VillageOasisCount($village->wid); ?></b> <a href="<?=\App\Routes::BUILD?>?id=<?php echo $id; ?>&land"><?php echo OASES; ?></a>.</p>
    ...
    if(isset($_GET['add'])) { 
            if($_GET['add'] == "reset") { 
                if($hero_info['level'] <= 3){ 
                    ...
                    $this->database->query("UPDATE " . TB_PREFIX . "hero SET `points` = '".(($hero_info['level']*5)+5)."' WHERE `uid` = '" . $session->uid . "'"); 
                    $this->database->query("UPDATE " . TB_PREFIX . "hero SET `attack` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $this->database->query("UPDATE " . TB_PREFIX . "hero SET `defence` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $this->database->query("UPDATE " . TB_PREFIX . "hero SET `attackbonus` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $this->database->query("UPDATE " . TB_PREFIX . "hero SET `defencebonus` = '0' WHERE `uid` = '" . $session->uid . "'"); 
                    $this->database->query("UPDATE " . TB_PREFIX . "hero SET `regeneration` = '0' WHERE `uid` = '" . $session->uid . "'"); 
            ...
            if($_GET['add'] == "off" && $hero_info['attack'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `attack` = `attack` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: ".\App\Routes::BUILD."?id=".$id."");
                    } 
                } 
            if($_GET['add'] == "deff" && $hero_info['defence'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `defence` = `defence` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: ".\App\Routes::BUILD."?id=".$id."");
                    } 
                } 
          if($_GET['add'] == "obonus" && $hero_info['attackbonus'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `attackbonus` = `attackbonus` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: ".\App\Routes::BUILD."?id=".$id."");
                    } 
                } 
          if($_GET['add'] == "dbonus" && $hero_info['defencebonus'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `defencebonus` = `defencebonus` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: ".\App\Routes::BUILD."?id=".$id."");
                    } 
                } 
          if($_GET['add'] == "reg" && $hero_info['regeneration'] < 100) { 
                    if($hero_info['points'] > 0) { 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `regeneration` = `regeneration` + 1 WHERE `uid` = '" . $session->uid . "'"); 
                        $this->database->query("UPDATE " . TB_PREFIX . "hero SET `points` = `points` - 1 WHERE `uid` = '" . $session->uid . "'"); 
                        header("Location: ".\App\Routes::BUILD."?id=".$id."");
                    } 
                } 

...
if (time() - (int) $_SESSION['time_p'] > 5) {
  $_SESSION['time_p'] = '';
  $_SESSION['error_p'] = '';
}

if ($_POST AND $_GET['action'] == 'change_capital') {
  $pass = $this->database->realEscapeString($_POST['pass']);
  $query = $this->database->query('SELECT * FROM `' . TB_PREFIX . 'users` WHERE `id` = ' . $session->uid);
  $data = $this->database->fetchAssoc($query);
  if($data['password'] == md5($pass)) {
    $query1 = $this->database->query('SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `owner` = ' . $session->uid . ' AND `capital` = 1');
    $data1 = $this->database->fetchAssoc($query1);
    $query2 = $this->database->query('SELECT * FROM `' . TB_PREFIX . 'fdata` WHERE `vref` = ' . $data1['wref']);
    $data2 = $this->database->fetchAssoc($query2);
    if($data2['vref'] != $village->wid) {
      for($i = 1; $i<=18; ++$i) {
        if($data2['f' . $i] > 10) {
          $query2 = $this->database->query('UPDATE `' . TB_PREFIX . 'fdata` SET `f' . $i . '` = 10 WHERE `vref` = ' . $data2['vref']);
        }
}
for($i=19; $i<=40; ++$i) {
        if($data2['f' . $i . 't'] == 34) {
          $query3 = $this->database->query('UPDATE `' . TB_PREFIX . 'fdata` SET `f' . $i . 't` = 0, `f' . $i . '` = 0 WHERE `vref` = ' . $data2['vref']);
        }
}
        
         for($i=19; $i<=40; ++$i) {
        if($data2['f' . $i . 't'] == 29 or $data2['f' . $i . 't'] == 30 or $data2['f' . $i . 't'] == 38 or $data2['f' . $i . 't'] == 39 or $data2['f' . $i . 't'] == 42) {
          $query3 = $this->database->query('UPDATE `' . TB_PREFIX . 'fdata` SET `f' . $i . 't` = 0, `f' . $i . '` = 0 WHERE `vref` = ' . $village->wid);
        }
}
$query3 = $this->database->query('UPDATE `' . TB_PREFIX . 'vdata` SET `capital` = 0 WHERE `wref` = ' . $data1['wref']);
$query4 = $this->database->query('UPDATE `' . TB_PREFIX . 'vdata` SET `capital` = 1 WHERE `wref` = ' . $village->wid);
}
        } else {
    $error = '<br /><font color="red">'.LOGIN_PW_ERROR.'</font><br />';
    $_SESSION['error_p'] = $error;
    $_SESSION['time_p'] = time();
    print '<script language="javascript">location.href="'.\App\Routes::BUILD.'?id=' . $building->getTypeField(26) . '&confirm=yes";</script>';
  }
}
?>
...
$test=$this->database->getAvailableExpansionTraining();
...
<?php
$query = $this->database->query('SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `owner` = ' . $session->uid . ' AND `capital` = 1');
$data = $this->database->fetchAssoc($query);
...
<form method="post" action="'.\App\Routes::BUILD.'?id=' . $building->getTypeField(26) . '&action=change_capital">
' . $_SESSION['error_p'] . '

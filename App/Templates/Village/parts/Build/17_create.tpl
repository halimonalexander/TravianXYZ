<form action="<?=\App\Routes::BUILD?>" method="post">
...
<?php
	...
	if($session->villages[$i-1] != $village->wid){
		$coor = $this->database->getCoor($session->villages[$i-1]);
		echo "<option value=\"".$session->villages[$i-1]."\" ".$select.">".$this->database->getVillageField($session->villages[$i-1],'name')." (".$coor['x']."|".$coor['y'].")</option>";
    }
	}
?>						</select>
					</td>

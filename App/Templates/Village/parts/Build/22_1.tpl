...
for ($i=2;$i<=9;$i++) {
	if (
      $technology->meetRRequirement($i) &&
      !$technology->getTech($i) &&
      !$technology->isResearch($i,1)
  ) { 
    	...
    	<tr>
    	    <td class="desc">
                  ...
                  />
					    ...
					    <div class="details">
					        <img class="r1" src="/img/x.gif" alt="Lumber" title="<?=LUMBER?>" /><?=$requirements['wood']?>|
                  <img class="r2" src="/img/x.gif" alt="Clay" title="<?=CLAY?>" /><?=$requirements['clay']?>|
                  <img class="r3" src="/img/x.gif" alt="Iron" title="<?=IRON?>" /><?=$requirements['iron']?>|
                  <img class="r4" src="/img/x.gif" alt="Crop" title="<?=CROP?>" /><?=$requirements['crop']?>|
                  <img class="clock" src="/img/x.gif" alt="duration" title="<?=DURATION?>" /><?=\App\Helpers\DatetimeHelper::secondsToTime(round($requirements['time'] * ($bid22[$village->resarray['f'.$id]]['attri'] / 100)/SPEED))?>
                  <?php
                  ...
                  if (
                      $requirements['wood'] > $village->maxstore ||
                      $requirements['clay'] > $village->maxstore ||
                      $requirements['iron'] > $village->maxstore
                  ) {
                    echo "<br><span class=\"none\">".EXPAND_WAREHOUSE1."</span></div></td>";
                    echo "<td class=\"act\">
                      <div class=\"none\">".EXPAND_WAREHOUSE."</div>
                    </td></tr>";
                  } elseif($requirements['crop'] > $village->maxcrop) {
                ...
                echo "</td>";
                    echo "<td class=\"act\">
					<a class=\"research\" href=\"".\App\Routes::BUILD."?id=$id&amp;a=$i&amp;c=".$session->mchecker."\">".RESEARCH."</a></td></tr>";
                }else{
                echo "</td>";
                    echo "<td class=\"act\">
...
if(count($acares) > 0) {
	...
	foreach($acares as $aca) {
		...
		echo "<tr><td class=\"desc\"><img class=\"unit u$unit\" src=\"img/x.gif\" alt=\"".$technology->getUnitName($unit)."\" title=\"".$technology->getUnitName($unit)."\" />".$technology->getUnitName($unit)."</td>";
			echo "<td class=\"dur\"><span id=\"timer$timer\">".\App\Helpers\DatetimeHelper::secondsToTime($aca['timestamp']-time())."</span></td>";

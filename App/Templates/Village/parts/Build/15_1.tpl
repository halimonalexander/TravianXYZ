...
if($session->access != BANNED){
    $this->database->delDemolition($village->wid);
    header("Location: ".\App\Routes::BUILD."?gid=15&ty=$ty&cancel=0&demolish=0");
}else{
...
if($session->access != BANNED){
    if($_REQUEST["type"] != null) {
        $type = $_REQUEST['type'];
        $this->database->addDemolition($village->wid,$type);
        $session->changeChecker();
        header("Location: ".\App\Routes::BUILD."?gid=15&ty=$type&cancel=0&demolish=0");
    }
}else{
header("Location: banned.php");
}
...
if($village->resarray['f'.$id] >= DEMOLISH_LEVEL_REQ) {
    ...
    $VillageResourceLevels = $this->database->getResourceLevel($village->wid);
    $DemolitionProgress = $this->database->getDemolition($village->wid);
    if (!empty($DemolitionProgress)) {
        $Demolition = $DemolitionProgress[0];
        echo "<b>";
        echo "<a href='".\App\Routes::BUILD."?id=".$_GET['id']."&ty=".$ty."&cancel=1'><img src='img/x.gif' class='del' title='".CANCEL."' alt='cancel'></a> ";
        echo "".DEMOLITION_OF." ".$building->procResType($VillageResourceLevels['f'.$Demolition['buildnumber'].'t']).": <span id=timer1>".\App\Helpers\DatetimeHelper::secondsToTime($Demolition['timetofinish']-time())."</span>";
            if($session->gold >= 2) {
            if($session->access!=BANNED){
...
echo "</b>";
        ...
        echo "
<form action=\"".\App\Routes::BUILD."?gid=15&amp;demolish=1&amp;cancel=0&amp;c=".$session->mchecker."\" method=\"POST\" style=\"display:inline\">

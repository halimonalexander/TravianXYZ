...
?>
 ...
 <div id="build" class="gid37">
		...
		if($this->database->numRows($hero) == 0){
	...
	<table id="distribution" cellpadding="1" cellspacing="1">
        ...
        if($this->database->numRows($hero) != 0 AND $hero_info['dead'] == 1 AND $hero_info['intraining'] == 0){
            include("37_revive.tpl");
        }
        if($this->database->numRows($hero) != 0 AND $hero_info['dead'] == 0 AND $hero_info['trainingtime'] <= time() AND $hero_info['inrevive'] == 0 AND $hero_info['intraining'] == 0){
            include("37_hero.tpl");
        }
        }
        include("upgrade.php"); ?>
